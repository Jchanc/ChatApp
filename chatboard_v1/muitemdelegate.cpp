...
void MuItemDelegate::paint(QPainter *painter, const QStyleOptionViewItem &option, const QModelIndex &index) const
{
    if (index.isValid()) {
        ...
        QRectF usrRect = QRect(iconRect.right()+5, iconRect.top(), rect.width()-10-iconRect.width(), 20);
        QRectF descRect = QRect(usrRect.left(), usrRect.bottom()+5, rect.width()-10-iconRect.width(), 20);

//        painter->drawImage(iconRect, QImage(itemData.iconPath));
        painter->setPen(QPen(Qt::black));
        painter->setFont(QFont("Microsoft Yahei", 10));
        painter->drawText(usrRect, itemData.usrName);

        painter->setPen(QPen(Qt::gray));
        painter->drawText(descRect, itemData.desc);
